...
Automatically updates vulnerable Python dependencies in `requirements.txt` files based on GitHub Advanced Security alerts in Azure DevOps.
...
3. **Updates** all `requirements.txt` files in your repository
4. **Creates** a pull request for each vulnerable dependency
5. **Documents** CVEs, severity levels, and changes

## Features

✅ **Focused on Dependencies** - Only processes package/dependency alerts  
✅ **Smart Package Detection** - Finds all requirements*.txt files automatically  
...
- **Python Only** - Currently supports `requirements.txt` files only
