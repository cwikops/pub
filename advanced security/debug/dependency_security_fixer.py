...
class DependencySecurityFixer:
    ...
    def __init__(self):
        ...
        self.dry_run = False
        self.debug_mode = False  # Enable for detailed alert dumps
    ...
    def extract_package_info(self, alert: Dict) -> Optional[Dict]:
        ...
        # Debug: Print full alert structure
        print(f"    DEBUG - Full alert structure:")
        print(f"      Title: {title}")
        print(f"      Type: {alert_type}")
        print(f"      Description (first 300 chars): {description[:300] if description else 'N/A'}")
        
        # Check physical locations for more context
        ...
        if physical_locations:
            ...
            if snippet:
                print(f"      Code snippet: {snippet[:100]}")
        
        # Check if there are toolComponents or rules with more info
        tool_components = alert.get('toolComponents', [])
        if tool_components:
            for component in tool_components[:1]:  # Just first one
                print(f"      Tool: {component.get('name', 'N/A')}")
        ...
        # Strategy 1: Try to extract from code snippet if available
        if physical_locations:
            snippet = physical_locations[0].get('region', {}).get('snippet', {}).get('text', '')
            if snippet:
                # Try to parse requirement line
                match = re.match(r'^\s*([a-zA-Z0-9_-]+)\s*([>=<~!]+)\s*([0-9.]+)', snippet)
                if match:
                    package_info['package_name'] = match.group(1)
                    package_info['current_version'] = match.group(3)
                    print(f"      → Extracted from snippet: {package_info['package_name']}@{package_info['current_version']}")
        
        # Strategy 2: Extract CVE
        cve_match = re.search(r'(CVE-\d{4}-\d+)', title + ' ' + description, re.IGNORECASE)
        if cve_match:
            package_info['cve'] = cve_match.group(1).upper()
            print(f"      → Found CVE: {package_info['cve']}")
        
        # Strategy 3: Enhanced package name patterns
        if not package_info['package_name']:
            package_patterns = [
                # Pattern: "package: name" or "Package name"
                r'[Pp]ackage[:\s]+([a-zA-Z0-9_-]+)',
                # Pattern: `package-name` or "package-name"  
                r'`([a-zA-Z0-9_-]+)`',
                r'"([a-zA-Z0-9_-]+)"',
                r'\'([a-zA-Z0-9_-]+)\'',
                # Pattern: "in package-name"
                r'in\s+([a-zA-Z0-9_-]+)',
                # Pattern: "Update package-name" or "Upgrade package-name"
                r'[Uu]pdate\s+([a-zA-Z0-9_-]+)',
                r'[Uu]pgrade\s+([a-zA-Z0-9_-]+)',
                # Pattern: "package-name vulnerability"
                r'([a-zA-Z0-9_-]+)\s+vulnerability',
                # Pattern: Package name at start
                r'^([a-zA-Z][a-zA-Z0-9_-]{2,})',
                # Common Python packages by name
                r'\b(requests|django|flask|numpy|pandas|pillow|urllib3|cryptography|pyyaml|jinja2|setuptools|wheel|pip|certifi|idna|charset-normalizer|click|werkzeug|itsdangerous|markupsafe)\b',
            ]
            
            exclude_words = ['the', 'has', 'been', 'found', 'detected', 'critical', 
                            'high', 'medium', 'low', 'security', 'vulnerable', 'package',
                            'dependency', 'update', 'upgrade', 'fix', 'alert']
            
            for pattern in package_patterns:
                match = re.search(pattern, title, re.IGNORECASE)
                if match:
                    potential_package = match.group(1).lower()
                    if potential_package not in exclude_words and len(potential_package) > 2:
                        package_info['package_name'] = potential_package
                        print(f"      → Found package name: {package_info['package_name']}")
                        break
            
            # If not in title, try description
            if not package_info['package_name'] and description:
                for pattern in package_patterns:
                    ...
                    if match:
                        ...
                        if potential_package not in exclude_words and len(potential_package) > 2:
                            package_info['package_name'] = potential_package
                            print(f"      → Found package name in description: {package_info['package_name']}")
                            break
                        
        # Strategy 4: Enhanced version extraction
        combined_text = title + ' ' + description
        
        # Try "X to Y" or "X.Y.Z -> A.B.C" pattern first (most explicit)
        to_pattern_match = re.search(r'(\d+\.\d+\.?\d*)\s+(?:to|->)\s+(\d+\.\d+\.?\d*)', combined_text)
        if to_pattern_match:
            package_info['current_version'] = to_pattern_match.group(1)
            package_info['fixed_version'] = to_pattern_match.group(2)
            print(f"      → Found versions (current->fixed): {package_info['current_version']} -> {package_info['fixed_version']}")
        
        # If not found, try other patterns
        if not package_info['fixed_version']:
            versions = re.findall(r'\d+\.\d+\.?\d*', combined_text)
            if versions:
                print(f"      → Found version numbers: {versions}")
                # Remove duplicates while preserving order
                versions = list(dict.fromkeys(versions))
                
                # Usually the last/highest version is the fixed one
                if len(versions) >= 1:
                    package_info['fixed_version'] = versions[-1]
                if len(versions) >= 2:
                    package_info['current_version'] = versions[0]
                print(f"      → Using versions: current={package_info['current_version']}, fixed={package_info['fixed_version']}")
        
        # Strategy 5: Check recommendations
        recommendations = alert.get('recommendations', [])
        if recommendations:
            print(f"      Checking {len(recommendations)} recommendation(s)")
            for rec in recommendations:
                rec_text = rec.get('text', '')
                if rec_text and ('upgrade' in rec_text.lower() or 'update' in rec_text.lower()):
                    print(f"        Recommendation: {rec_text[:100]}...")
                    version_match = re.search(r'(\d+\.\d+\.?\d*)', rec_text)
                    if version_match and not package_info['fixed_version']:
                        package_info['fixed_version'] = version_match.group(1)
                        print(f"        → Fixed version from recommendation: {package_info['fixed_version']}")
        
        # Strategy 6: If we have package name but no current version, search requirements files
        if package_info['package_name'] and not package_info['current_version']:
            print(f"      Searching requirements files for {package_info['package_name']}...")
            current_ver = self._find_package_in_requirements(package_info['package_name'])
            if current_ver:
                package_info['current_version'] = current_ver
                print(f"      → Found current version in files: {current_ver}")
        
        print(f"    ═══ Final extraction result ═══")
        print(f"      Package: {package_info['package_name'] or 'NOT FOUND'}")
        print(f"      Current: {package_info['current_version'] or 'N/A'}")
        print(f"      Fixed: {package_info['fixed_version'] or 'NOT FOUND'}")
        print(f"      CVE: {package_info['cve'] or 'N/A'}")
        
        return package_info if package_info['package_name'] else None
    
    def _find_package_in_requirements(self, package_name: str) -> Optional[str]:
        """Find package version in existing requirements files"""
        try:
            req_files = self.find_requirements_files()
            print(f"        Checking {len(req_files)} requirements file(s)")
                    ...
                    with open(req_file, 'r') as f:
                        for line in f:
                            ...
                            if line and not line.startswith('#'):
                                ...
                                match = re.match(rf'^{re.escape(package_name)}([>=<~!]+)([0-9.]+)', line, re.IGNORECASE)
                                if match:
                                    print(f"        Found in {req_file}: {line}")
    ...
    def find_requirements_files(self) -> List[str]:
        ...
        for root, dirs, files in os.walk('.'):
            ...
            dirs[:] = [d for d in dirs if d not in ['.git', 'node_modules', 'venv', '.venv', '__pycache__', '.uv']]
    ...
    def update_requirement(self, file_path: str, package_name: str, 
        ...
        """Update a package version in requirements.txt file using uv"""
        try:
            # Check if this is a pyproject.toml file
            if file_path.endswith('pyproject.toml'):
                return self._update_pyproject_toml(file_path, package_name, fixed_version)
            
            # For requirements.txt files, use uv pip compile approach
            ...
            if updated:
                ...
                # Use uv to validate and lock the requirements
                print(f"    Validating with uv...")
                self._validate_with_uv(file_path)
    ...
    def _update_pyproject_toml(self, file_path: str, package_name: str, fixed_version: str) -> bool:
        """Update package version in pyproject.toml"""
        try:
            with open(file_path, 'r') as f:
                content = f.read()
            
            # Pattern to match package in dependencies
            # e.g., "requests>=2.25.0" or "requests==2.25.0"
            pattern = rf'("{package_name}[^"]*")'
            match = re.search(pattern, content, re.IGNORECASE)
            
            if match:
                old_dep = match.group(1)
                new_dep = f'"{package_name}=={fixed_version}"'
                new_content = content.replace(old_dep, new_dep)
                
                with open(file_path, 'w') as f:
                    f.write(new_content)
                
                print(f"    Updated pyproject.toml: {old_dep} -> {new_dep}")
                return True
    ...
    def _validate_with_uv(self, requirements_file: str) -> bool:
        """Validate requirements file with uv"""
        try:
            ...
            if result.returncode != 0:
                print(f"    Warning: uv not available, skipping validation")
                return True
            
            # Use uv pip compile to validate
            result = subprocess.run(
                ['uv', 'pip', 'compile', requirements_file, '--quiet'],
                capture_output=True,
                text=True,
                timeout=30
            )
            ...
            if result.returncode == 0:
                print(f"    ✓ Requirements validated with uv")
                return True
            else:
                print(f"    Warning: uv validation failed: {result.stderr}")
                return True  # Don't fail the update if validation fails
                
        except FileNotFoundError:
            print(f"    Info: uv not installed, skipping validation")
            return True
        except subprocess.TimeoutExpired:
            print(f"    Warning: uv validation timed out")
            return True
        except Exception as e:
            print(f"    Warning: uv validation error: {e}")
            return True
    
    def _generate_lockfile_with_uv(self, updated_files: List[str]) -> bool:
        """Generate or update lockfile using uv"""
        try:
            # Check if uv is available
            result = subprocess.run(
                ['uv', '--version'],
            ...
            if result.returncode != 0:
                print(f"    Info: uv not available, skipping lockfile generation")
                return False
            
            print(f"    Generating lockfile with uv...")
            ...
            if not primary_file:
                primary_file = updated_files[0]
            
            # Generate requirements.lock or uv.lock
            if primary_file == 'pyproject.toml':
                # For pyproject.toml, use uv lock
                result = subprocess.run(
                    ['uv', 'lock'],
                    capture_output=True,
                    text=True,
                ...
                if result.returncode == 0:
                    print(f"    ✓ Generated uv.lock")
                    # Add lockfile to git
                    if os.path.exists('uv.lock'):
                        subprocess.run(['git', 'add', 'uv.lock'], check=False)
                    return True
                else:
                    print(f"    Warning: uv lock failed: {result.stderr}")
                    return False
            else:
                # For requirements.txt, use uv pip compile
                lockfile = primary_file.replace('.txt', '.lock')
                if lockfile == primary_file:
                    lockfile = 'requirements.lock'
                
                result = subprocess.run(
                    ['uv', 'pip', 'compile', primary_file, '-o', lockfile],
                    capture_output=True,
                    text=True,
                    timeout=60
                )
                ...
                if result.returncode == 0:
                    print(f"    ✓ Generated {lockfile}")
                    # Add lockfile to git
                    if os.path.exists(lockfile):
                        subprocess.run(['git', 'add', lockfile], check=False)
                    return True
                else:
                    print(f"    Warning: Failed to generate lockfile: {result.stderr}")
                    return False
                    
        except FileNotFoundError:
            print(f"    Info: uv not installed, skipping lockfile generation")
            return False
        except subprocess.TimeoutExpired:
            print(f"    Warning: uv lockfile generation timed out")
            return False
        except Exception as e:
            print(f"    Warning: lockfile generation error: {e}")
            return False
    ...
    def process_dependency_alert(self, alert: Dict) -> bool:
        ...
        # Debug: Dump full alert if debug mode enabled
        if self.debug_mode:
            print(f"  DEBUG - Full alert JSON:")
        ...
        try:
            ...
            # Generate/update lockfile with uv if available
            self._generate_lockfile_with_uv(files_updated)
...
def main():
    ...
    parser.add_argument('--debug', action='store_true',
                       help='Enable debug mode (verbose output with full alert JSON)')
